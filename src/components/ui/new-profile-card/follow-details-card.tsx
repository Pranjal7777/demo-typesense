...
const FollowDetailsCard:FC<FollowDetailsCardProps> = ({firstName, lastName, profilePicUrl, username, buttonText, buttonType, buttonClassName,accountId, refetch, handleUnfollow}) => {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-bg-septenary-light dark:border-bg-secondary-dark">
      ...
      <Button disabled={isFollowLoading || isUnFollowLoading} isLoading={isFollowLoading || isUnFollowLoading} onClick={onFollowUnfollow} className={appClsx("w-[76px] mt-0 mb-0 h-[30px] text-xs", buttonClassName)} buttonType={buttonType}>
        {buttonText}
      </Button>
    </div>
  );
