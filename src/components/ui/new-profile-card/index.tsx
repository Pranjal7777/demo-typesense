...
const NewProfileCard: FC<Props> = ({
  ...
  return (
    <div className={appClsx('w-full flex flex-col items-center md:items-start gap-1', cardClass)}>
      ...
      {!HIDE_SELLER_FLOW && (
        <Button onClick={followButtonHandler} buttonType={buttonType} className={buttonClass}>
          {buttonText}
        </Button>
      )}
      {!HIDE_SELLER_FLOW && showFollowingSection && (
        <div
          className={appClsx(
            'w-full flex gap-4 justify-center md:justify-start text-sm font-medium text-text-secondary-dark',
            followingSectionClass
          )}
        >
