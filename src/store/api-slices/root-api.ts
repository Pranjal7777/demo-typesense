...
// Utility function to fetch a guest token
...
import { setRemoveUserDataDispatch, setGuestTokenDispatch, setUpdateAccessTokenDispatch } from '../slices/auth-slice';
import { GUEST_REFRESH_TOKEN_URL } from '@/api/endpoints';
import { GetGuestTokenConfig } from '../types';
import { removeCookie, setCookie } from '@/utils/cookies';
...
const fetchGuestToken = async () => {
  const response = await fetch(`${BASE_API_URL + AUTH_URL_V1}/guestLogin`, {
    method: 'POST',
    headers: {
      Authorization: `${guestGeneratedToken}`,
      'Content-Type': 'application/json',
  ...
  setCookie(ACCESS_TOKEN, data.data.token?.accessToken, { expires: 1 });
  setCookie(REFRESH_ACCESS_TOKEN, data.data.token?.refreshToken, { expires: 1 });
  setGuestTokenDispatch(data.data.token);

  return data.data.token;
};

// Utility function for refreshing tokens
const refreshAccessToken = async (refreshTokenPayload: { accessToken: string; refreshToken: string }) => {
  const response = await fetch(`${BASE_API_URL}${AUTH_URL_V1}/${GUEST_REFRESH_TOKEN_URL}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: guestGeneratedToken,
...
const baseQuery = fetchBaseQuery({
  ...
  prepareHeaders: (headers) => {
    const authToken = Cookies.get(ACCESS_TOKEN);
    const storedLocation = localStorage.getItem('myLocation');
    const geoLocation = storedLocation ? JSON.parse(storedLocation) : null;
    const myIpAddress = localStorage.getItem('ipAddress');
    ...
    if (authToken) {
      ...
      headers.set('City', geoLocation?.city || DEFAULT_LOCATION.city);
      headers.set('Country', geoLocation?.country || DEFAULT_LOCATION.country);
      headers.set('Ipaddress', myIpAddress || DEFAULT_LOCATION.ip);
      headers.set('Latitude', geoLocation?.latitude || DEFAULT_LOCATION.latitude);
      headers.set('Longitude', geoLocation?.longitude || DEFAULT_LOCATION.longitude);
    }
    return headers;
...
const baseQueryWithAuth: BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> = async (
  ...
  if (result.error && (result.error.status === 401  || result.error.status === 406)) {
    ...
    try {
      ...
      if (!refreshTokenPayload.refreshToken) throw new Error('Missing refresh token');

      await refreshAccessToken(refreshTokenPayload);
      ...
      try {
        ...
        console.log('Failed to fetch guest token:', guestTokenError);
      }
      window.location.href = '/';
      // return { error: 'Redirecting to home due to authentication failure' };
    }
  }
